...
(defn safe-add [k c]
  (char (+ (int \a) (mod (- (+ k (int c)) (int \a)) 26))))
...
(def success-text "Well done! Your result is correct. As we are always in need for talents like you, make the decision to apply today. We offer a wide range of exciting roles here at ICA. Discover our very latest vacancies.")
...
(defn on-message [payload]
  ...
  (let [sender-id (get-in payload [:sender :id])
    ...
    (cond
      (s/includes? (s/lower-case message-text) ciphertext) (fb/send-message sender-id (fb/text-message success-text))
